...
Usage-host must have PlatformIO CLI and pyserial installed:
   $ python3 flash_code_and_monitor_w_doc.py
...
def upload_prebuilt_binary(port, project_path, build_env_name):
    ...
    """
    bootloader_bin_path, partitions_bin_path, firmware_bin_path = find_files_in_incomplete_directory(
        f"{project_path}/.pio", ["bootloader.bin", "partitions.bin", "firmware.bin"], subdir=build_env_name
    )
    boot_app0_bin_path = find_files_in_incomplete_directory(
        "/home/pfr/.platformio/packages/framework-arduinoespressif32/", "boot_app0.bin"
    )
    ...
    esptool_path, python_interp_path = find_files_in_incomplete_directory(
        "/home/pfr/.platformio", ["esptool.py", "python"]
    )
